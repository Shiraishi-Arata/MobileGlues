...
void init_settings() {
    ...
    int ylVersion = 0;
    GetEnvVarInt("YUKARI_VERSION_CODE", &ylVersion, 0);

    LOG_V("MG_DIR_PATH = %s", mg_directory_path ? mg_directory_path : "(default)")

    // MobileGlues only runs on Yukari Launcher
    if (ylVersion == 0) {
        LOG_V("Unsupported launcher detected. MobileGlues only runs on Yukari Launcher.")
